...
$config = array();
$config['Author'] = '';
$config['Channel Title'] = 'My Youtube Wall';
$config['Channel Description'] = 'YoutubeWall by XooFoo';
...
function mulit($count, $page, $perpage) {
	...
	$i = 1;
	$pager = "<nav id='mulit'><a class='bulbe' href='?dir=$dir&page=1' title='First page'><em>First</em></a>";
	while($allpage) {
		$class = '';
		if ($i == $page) {
			$class = ' class="current"'; 
		...
		if ($dir) {
			...
			$pager .= "<a class='bulbe' href='?page=$i' title='Go to page $i'><em$class>";    
		}
		$pager .= "$i";
		$pager .= "</em></a>";
		$i ++;
	...
	$next = $page + 1;
	$pager .= "<a class='bulbe' href='?dir=$dir&page=$last' title='Last page'><em>Last</em></a>";
	$pager .= "<a class='bulbe' href='?dir=$dir&page=$next' title='Next page'><em>NEXT</em></a></nav>";
	return $pager;
}
// Convert Contact
...
<head>
	...
	<meta name="generator" content="YouTubeWall" />
	...
	<link rel="help" title="YouTubeWall Channel" href="https://github.com/krisxoofoo/YouTubeWall/" />
	<style type="text/css">
		html { height: 100%;}
		* {  margin: 0; padding: 0;}
...
<?php } ?>
	...
	<style type="text/css">
		/* GALLERY */
		#pbOverlay { background: #0b0b0b url('<?php echo $set['script name']; ?>?symbol=overlay') repeat !important; }
...
<?php if($config['Embedded Script'] == 'off') { ?>
	<?php if(!empty($config['Header Image'])) { ?>
		<header>
			...
			<span>
			<?php if(!empty($config['Channel Title'])) { ?><?php if(!empty($config['Home Page'])) { ?><a class="bulbe" href="<?php echo $config['Home Page']; ?>" title="<?php echo $config['Channel Description']; ?>"><?php } ?><?php echo $config['Channel Title']; ?><?php if(!empty($config['Home Page'])) { ?></a><?php } ?><?php } ?>
			</span></h1>
		</header>
	<?php } ?>
<?php } ?>
...
<?php
	...
	{ $key = explode(";",$videos[$i]);
		echo('<li class="figure"><a class="bulbe"  rel="video" href="http://www.youtube.com/watch?v='.$key[0].'" title="'.$key[1].'"><img src="http://i1.ytimg.com/vi/'.$key[0].'/default.jpg" alt="'.$key[1].'" /><span class="figcaption">'.$key[1].'</span></a></li>');
	}
?>
	</ul>
		</div>
